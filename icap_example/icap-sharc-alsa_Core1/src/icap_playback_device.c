...
int create_icap_sharc_alsa_playback_endpoint(void)
{
	...
	rpmsg_ept = rpmsg_lite_create_ept(
		...
		&icap_call_back, (void *)&icap_sharc_alsa_playback,
...
int remove_icap_sharc_alsa_playback_endpoint(void)
{
	...
	ret = rpmsg_ns_announce(
		...
		icap_sharc_alsa_playback.transport.rpmsg_ept, "sharc-alsa",
		RL_NS_DESTROY);
...
int32_t icap_sharc_alsa_playback_get_subdevice_features(
	...
	struct icap_subdevice_features *features)
{
	...
	memcpy(features, &sport_playback_device_features,
	       sizeof(struct icap_subdevice_features));
	return 0;
}

int32_t icap_sharc_alsa_playback_start(struct icap_instance *icap,
				       uint32_t subdev_id)
{
	if (subdev_id >= 1) {
		return -ICAP_ERROR_INVALID;
...
int32_t icap_sharc_alsa_playback_stop(struct icap_instance *icap,
				      uint32_t subdev_id)
{
	if (subdev_id >= 1) {
		return -ICAP_ERROR_INVALID;
...
int32_t icap_sharc_alsa_playback_add_src(struct icap_instance *icap,
					 struct icap_buf_descriptor *buf)
{
	memcpy(&icap_sharc_alsa_playback_buffer.descr, buf,
...
int32_t icap_sharc_alsa_playback_add_dst(struct icap_instance *icap,
					 struct icap_buf_descriptor *buf)
{
	return -ICAP_ERROR_NOT_SUP;
}

int32_t icap_sharc_alsa_playback_frag_ready_response(struct icap_instance *icap,
						     int32_t buf_id)
{
...
struct icap_device_callbacks icap_sharc_alsa_playback_callbacks = {
	...
	.get_subdevice_features =
		icap_sharc_alsa_playback_get_subdevice_features,
...
int init_sharc_alsa_playback(void)
{
	...
	icap_sharc_alsa_playback.transport.rpmsg_ept =
		&icap_sharc_alsa_playback_endpoint_context.ept;
	ret = icap_device_init(&icap_sharc_alsa_playback, "playback",
			       &icap_sharc_alsa_playback_callbacks, NULL);
	if (ret) {
		return ret;
	}
