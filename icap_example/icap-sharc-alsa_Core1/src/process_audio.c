...
static SYSTEM_AUDIO_TYPE
    LINUX_OUT_BUFFER[2][LINUX_AUDIO_OUT_CHANNELS * SYSTEM_BLOCK_SIZE] = { 0 };

#pragma section("seg_l2_dmda_bw")
static SYSTEM_AUDIO_TYPE
    LINUX_IN_BUFFER[2][LINUX_AUDIO_IN_CHANNELS_MAX * SYSTEM_BLOCK_SIZE] = { 0 };
    ...
    ROUTE_INFO *routeInfo, unsigned numRoutes)
{
	...
	if(channel_route==1)
	{
		...
		{
			for (int in_ch = 4, op_ch = 0; in_ch < sink->numChannels; in_ch++, op_ch++)
			{
				out_ptr[fr * sink->numChannels + op_ch] = ip_ptr[fr * sink->numChannels + in_ch];
			}

			for (int op_ch = 12; op_ch < sink->numChannels; op_ch++)
			{
	...
	if(icap_sharc_alsa_playback_buffer.in_use==1)
	{
	    ...
	    for (int i=0; i<SYSTEM_BLOCK_SIZE; i++) {
	        for (int j=0; j<LINUX_AUDIO_OUT_CHANNELS; ++j) {
	            put_s32(&icap_sharc_alsa_record_buffer, out_ptr[i* LINUX_AUDIO_OUT_CHANNELS + j]);
	        }
	    }
	}
    ...
    void *data, bool flush, bool invalidate)
{
    ...
    streamInfo->altData = streamInfo->data != NULL ? streamInfo->data : data;
    ...
    void *data, bool flush)
{
    ...
    setStreamInfo(streamID, numChannels, numFrames, wordSize, data, flush, false);

    linuxAudioIn(LINUX_IN_BUFFER[pingPong], (linux_in_channels * sizeof(SYSTEM_AUDIO_TYPE) * SYSTEM_BLOCK_SIZE) , context);

    /* Configure "clockless" streams */
    setStreamInfo(
            STREAM_ID_LINUX_IN, linux_in_channels,
            SYSTEM_BLOCK_SIZE, sizeof(SYSTEM_AUDIO_TYPE),
            LINUX_IN_BUFFER[pingPong], false, false);
