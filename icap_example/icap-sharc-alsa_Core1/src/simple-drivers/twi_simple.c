...
#define TWI_SIMPLE_IRQ_MASK \
	(BITM_TWI_ISTAT_TXSERV | BITM_TWI_ISTAT_RXSERV | BITM_TWI_ISTAT_MCOMP)
...
#define TX_FIFO_FULL                                             \
	((*twi->pREG_TWI_FIFOSTAT & BITM_TWI_FIFOSTAT_TXSTAT) == \
	 ENUM_TWI_FIFOSTAT_TX_FIFO_FULL)
#define ENUM_TWI_FIFOSTAT_RX_FIFO_EMPTY (0u << BITP_TWI_FIFOSTAT_RXSTAT)
#define RX_FIFO_EMPTY                                            \
	((*twi->pREG_TWI_FIFOSTAT & BITM_TWI_FIFOSTAT_RXSTAT) == \
	 ENUM_TWI_FIFOSTAT_RX_FIFO_EMPTY)
...
static TWI_SIMPLE_RESULT twi_xfer(sTWI *twi, uint8_t address,
				  TWI_SIMPLE_XFER_TYPE xferType, uint8_t *out,
			    ...
			    uint16_t outLen)
{
	...
	result = twi_xfer(twiHandle, address, TWI_SIMPLE_WRITE, out, outLen,
			  NULL, 0);
			   ...
			   uint16_t inLen)
{
	...
	result = twi_xfer(twiHandle, address, TWI_SIMPLE_READ, NULL, 0, in,
			  inLen);
				...
				uint16_t outLen, uint8_t *in, uint16_t inLen)
{
	...
	result = twi_xfer(twiHandle, address, TWI_SIMPLE_WRITEREAD, out, outLen,
			  in, inLen);
				 ...
				 uint16_t out2Len)
{
	...
	result = twi_xfer(twiHandle, address, TWI_SIMPLE_WRITEWRITE, out,
			  outLen, out2, out2Len);
...
static void twi_initRegs(sTWI *twi)
{
	...
	*twi->pREG_TWI_FIFOCTL = ENUM_TWI_FIFOCTL_RXTWOBYTE |
				 ENUM_TWI_FIFOCTL_TXTWOBYTE;
...
TWI_SIMPLE_RESULT twi_init(void)
{
	...
	for (port = TWI0; port < TWI_END; port++) {
		...
		if (port == TWI0) {
			...
			twi->pREG_TWI_FIFOSTAT =
				(volatile uint16_t *)pREG_TWI0_FIFOSTAT;
			...
			twi->pREG_TWI_FIFOSTAT =
				(volatile uint16_t *)pREG_TWI1_FIFOSTAT;
			...
			twi->pREG_TWI_FIFOSTAT =
				(volatile uint16_t *)pREG_TWI2_FIFOSTAT;
...
TWI_SIMPLE_RESULT twi_open(TWI_SIMPLE_PORT port, sTWI **twiHandle)
{
	...
	if (result == TWI_SIMPLE_SUCCESS) {
		...
		if (status == ADI_INT_SUCCESS) {
			...
			*twi->pREG_TWI_CTL |=
				(TWI_SIMPLE_PRESCALE << BITP_TWI_CTL_PRESCALE) |
				ENUM_TWI_CTL_EN;
	...
	*twi->pREG_TWI_ISTAT = istat;
	...
	if (istat & TWI_SIMPLE_IRQ_MASK) {
		if (istat & BITM_TWI_ISTAT_TXSERV) {
             ...
             */
			if ((twi->xferType == TWI_SIMPLE_WRITE) &&
			    (twi->longXfer) && (twi->slen < TWI_MAX_DCNT)) {
				mstrctl = *twi->pREG_TWI_MSTRCTL &
					  ~BITM_TWI_MSTRCTL_DCNT;
		...
		if (istat & BITM_TWI_ISTAT_MCOMP) {
			...
			xferDone =
				(*twi->pREG_TWI_MSTRSTAT & TWI_SIMPLE_ERRORS);
			...
			if ((!xferDone) &&
			    (twi->xferType == TWI_SIMPLE_WRITEREAD)) {
				if (twi->rlen > TWI_MAX_DCNT) {
					len = 0xFF;
					twi->longXfer = 1;
				} else {
				...
				mstrctl = *twi->pREG_TWI_MSTRCTL;
				mstrctl &= ~(BITM_TWI_MSTRCTL_RSTART |
					     BITM_TWI_MSTRCTL_DIR);
				mstrctl |= (len << BITP_TWI_MSTRCTL_DCNT) |
					   ENUM_TWI_MSTRCTL_RX;
				*twi->pREG_TWI_MSTRCTL = mstrctl;
