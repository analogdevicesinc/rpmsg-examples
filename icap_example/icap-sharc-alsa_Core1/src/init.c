...
GPIO_CONFIG gpioPins[GPIO_PIN_MAX] = {
    ...
    { ADI_GPIO_PORT_A, ADI_GPIO_PIN_13,  ADI_GPIO_DIRECTION_OUTPUT, 0 }, // GPIO_PIN_DEBUG_0
#endif
};
    ...
    for (i = 0; i < 2; i++) {
        if (!cfg->dataBuffers[i]) {
            cfg->dataBuffers[i] = umm_malloc_heap_aligned(
                UMM_L2_SPORT_HEAP, dataBufferSize, ADI_CACHE_LINE_LENGTH);
        if (cfg->dataBuffers[i] == NULL) {
            syslog_printf("Failed to allocate SPORT buffer of size %#x\n", dataBufferSize);
